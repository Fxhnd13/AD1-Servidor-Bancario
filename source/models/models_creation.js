...
const syncronization = async (req, res) => {
    await Request.hasOne(Account_Request,{foreignKey: 'id_request'});
    await Person.hasMany(Account_Request,{foreignKey: 'cui'});
    await Account_Type.hasMany(Account_Request,{foreignKey: 'id_account_type'});
    await Account_Request.belongsTo(Request,{foreignKey: 'id_request'});
    await Account_Request.belongsTo(Person,{foreignKey: 'cui'});
    await Account_Request.belongsTo(Account_Type,{foreignKey: 'id_account_type'});
    //----------------------------------------------------------------------------
    await Person.hasMany(Account,{foreignKey: 'cui'});
    await Account_Type.hasMany(Account,{foreignKey: 'id_account_type'});
    await Account.belongsTo(Person,{foreignKey: 'cui'});
    await Account.belongsTo(Account_Type,{foreignKey: 'id_account_type'});
    //----------------------------------------------------------------------------
    await Bank_User.hasOne(Active_Session_Log,{foreignKey: 'username'});
    await Active_Session_Log.belongsTo(Bank_User,{foreignKey: 'username'});
    //----------------------------------------------------------------------------
    await Bank_User_Type.hasMany(Bank_User,{foreignKey: 'user_type'});
    await Bank_User.belongsTo(Bank_User_Type,{foreignKey: 'user_type'});
    await Person.hasMany(Bank_User,{foreignKey: 'cui'});
    await Bank_User.belongsTo(Person,{foreignKey: 'cui'});
    //----------------------------------------------------------------------------
    await Request.hasOne(Card_Cancellation_Request,{foreignKey: 'id_request'});
    await Card_Cancellation_Request.belongsTo(Request,{foreignKey: 'id_request'});
    //----------------------------------------------------------------------------
    await Card.hasMany(Card_Payment_Log,{foreignKey: 'id_card'});
    await Card_Payment_Log.belongsTo(Card,{foreignKey: 'id_card'});
    //----------------------------------------------------------------------------
    await Person.hasMany(Card,{foreignKey: 'cui'});
    await Card.belongsTo(Person,{foreignKey: 'cui'});
    //----------------------------------------------------------------------------
    await Request.hasOne(Credit_Card_Request,{foreignKey: 'id_request'});
    await Credit_Card_Request.belongsTo(Request,{foreignKey: 'id_request'});
    await Person.hasMany(Credit_Card_Request,{foreignKey: 'cui'});
    await Credit_Card_Request.belongsTo(Person,{foreignKey: 'cui'});
    //----------------------------------------------------------------------------
    await Card.hasOne(Credit_Card,{foreignKey: 'id_card'});
    await Credit_Card.belongsTo(Card,{foreignKey: 'id_card'});
    await Credit_Card_Type.hasMany(Credit_Card,{foreignKey: 'id_credit_card_type'});
    await Credit_Card.belongsTo(Credit_Card_Type,{foreignKey: 'id_credit_card_type'});
    //----------------------------------------------------------------------------
    await Request.hasOne(Debit_Card_Request,{foreignKey: 'id_request'});
    await Debit_Card_Request.belongsTo(Request,{foreignKey: 'id_request'});
    await Account.hasMany(Debit_Card_Request,{foreignKey: 'id_account'});
    await Debit_Card_Request.belongsTo(Account,{foreignKey: 'id_account'});
    //----------------------------------------------------------------------------
    await Card.hasOne(Debit_Card,{foreignKey: 'id_card'});
    await Debit_Card.belongsTo(Card,{foreignKey: 'id_card'});
    await Account.hasMany(Debit_Card,{foreignKey: 'id_account'});
    await Debit_Card.belongsTo(Account,{foreignKey: 'id_account'});
    //----------------------------------------------------------------------------
    await Account.hasMany(Deposit,{foreignKey: 'destination_account'});
    await Deposit.belongsTo(Account,{foreignKey: 'destination_account'});
    await Bank_User.hasMany(Deposit,{foreignKey: 'responsible_username'});
    await Deposit.belongsTo(Bank_User,{foreignKey: 'responsible_username'});
    //----------------------------------------------------------------------------
    await Bank_User.hasOne(Email,{foreignKey: 'username'});
    await Email.belongsTo(Bank_User,{foreignKey: 'username'});
    //----------------------------------------------------------------------------
    await Request.hasOne(Loan_Request,{foreignKey: 'id_request'});
    await Loan_Request.belongsTo(Request,{foreignKey: 'id_request'});
    await Person.hasMany(Loan_Request,{foreignKey: 'cui'});
    await Loan_Request.belongsTo(Person,{foreignKey: 'cui'});
    await Person.hasMany(Loan_Request,{foreignKey: 'guarantor_cui'});
    await Loan_Request.belongsTo(Person,{foreignKey: 'guarantor_cui'});
    //----------------------------------------------------------------------------
    await Credit_Card.hasMany(Payment_Delay,{foreignKey: 'id_card'});
    await Payment_Delay.belongsTo(Credit_Card,{foreignKey: 'id_card'});
    //----------------------------------------------------------------------------
    await Loan.hasMany(Payment_Log,{foreignKey: 'id_loan'});
    await Payment_Log.belongsTo(Loan,{foreignKey: 'id_loan'});
    //----------------------------------------------------------------------------
    await Request.hasOne(Update_Data_Request,{foreignKey: 'id_request'});
    await Update_Data_Request.belongsTo(Request,{foreignKey: 'id_request'});
    await Person.hasMany(Update_Data_Request,{foreignKey: 'cui'});
    await Update_Data_Request.belongsTo(Person,{foreignKey: 'cui'});
    //----------------------------------------------------------------------------
    await Account.hasMany(Withdrawal,{foreignKey: 'origin_account'});
    await Withdrawal.belongsTo(Account,{foreignKey: 'origin_account'});
    await Bank_User.hasMany(Withdrawal,{foreignKey: 'responsible_username'});
    await Withdrawal.belongsTo(Bank_User,{foreignKey: 'responsible_username'});
    //----------------------------------------------------------------------------
    ...
    await Card.bulkCreate([
        ...
        {pin: 1010, cui: 1000000000002, card_type: 1, expiration_date: '2025-12-12', active: true},
